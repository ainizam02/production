...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 15, 2025 at 03:51 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `restore_inventory_for_production` (IN `schedule_id_param` INT, OUT `success` BOOLEAN, OUT `error_message` TEXT)   BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE ingredient_product_id VARCHAR(10);
    DECLARE required_qty DECIMAL(10,2);
    DECLARE batch_count INT;
    ...
    OPEN ingredient_cursor;
    restore_loop: LOOP
        FETCH ingredient_cursor INTO ingredient_product_id, required_qty, batch_count;
        IF done THEN
            LEAVE restore_loop;
        END IF;
        
        SET total_required = required_qty * batch_count;
        
        -- Restore inventory
        UPDATE roti_seri_bakery_inventory.products 
        SET stock_quantity = stock_quantity + total_required,
            last_updated = NOW()
        WHERE product_id = ingredient_product_id;
        ...
        ('production_system', 'stock_increase', ingredient_product_id, 
         CONCAT('Production cancellation restore: Schedule ID ', schedule_id_param, ' - Quantity: ', total_required), 
         '127.0.0.1');
...
(13, 17, '2025-06-16', 1, 1, 'Pending', 1);

--
-- Triggers `tbl_schedule`
--
DELIMITER $$
CREATE TRIGGER `schedule_status_change` AFTER UPDATE ON `tbl_schedule` FOR EACH ROW BEGIN
    DECLARE proc_success BOOLEAN DEFAULT FALSE;
    DECLARE proc_error TEXT DEFAULT '';
    
    -- When schedule status changes from Pending to In Progress, deduct inventory
    IF OLD.schedule_status = 'Pending' AND NEW.schedule_status = 'In Progress' THEN
        CALL deduct_inventory_for_production(NEW.schedule_id, proc_success, proc_error);
        
        -- If deduction failed, log the error but don't block the update
        IF NOT proc_success THEN
            INSERT INTO roti_seri_bakery_inventory.inventory_logs 
            (user_id, action, item_id, action_details, ip_address)
            VALUES 
            ('production_system', 'error', CAST(NEW.schedule_id AS CHAR), 
             CONCAT('Failed to deduct inventory: ', COALESCE(proc_error, 'Unknown error')), '127.0.0.1');
        END IF;
    END IF;
    
    -- When schedule is cancelled, restore inventory (only if it was previously In Progress)
    IF OLD.schedule_status = 'In Progress' AND NEW.schedule_status = 'Cancelled' THEN
        CALL restore_inventory_for_production(NEW.schedule_id, proc_success, proc_error);
        ...
        IF NOT proc_success THEN
            ...
            VALUES 
            ('production_system', 'error', CAST(NEW.schedule_id AS CHAR), 
             CONCAT('Failed to restore inventory: ', COALESCE(proc_error, 'Unknown error')), '127.0.0.1');
        END IF;
    END IF;
END
$$
...
(38, 13, 1, '2025-06-15 21:09:02.629436'),
(39, 13, 4, '2025-06-15 21:09:02.629610');
...
--
ALTER TABLE `tbl_schedule`
  MODIFY `schedule_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;

--
-- AUTO_INCREMENT for table `tbl_schedule_assignments`
--
ALTER TABLE `tbl_schedule_assignments`
  MODIFY `sa_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=42;

--
-- AUTO_INCREMENT for table `tbl_schedule_equipment`
--
ALTER TABLE `tbl_schedule_equipment`
  MODIFY `se_id` int(6) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=40;
